...
| **P7.5** ✅ | 見 `docs/CLOUD_SETUP.md`「對外公開網域＋HTTPS」 | 完成 2026-09-07：https://141-147-176-204.sslip.io ，Let's Encrypt 真憑證 |

P7.5 正好接上 `CLOUD_SETUP.md` 裡卡住的地方。**P7.4 沒有全綠之前不要做 P7.5**，
那等於把沒守門的服務放上公網。

**（2026-09-07：P7.0–P7.5 全部完成，帳號系統結案。）**
...
## P7.3 落地紀錄（2026-09-07 完成）

### 權限守門的接縫先建好了

新增 `src/lib/auth/access.ts` 的 `requireTripAccess(tripId, minimumRole)`。
角色比較一律問「夠不夠」而不是「等不等於」（OWNER > EDITOR > VIEWER），
新增角色時不必回頭改每個呼叫點。**P7.4 要讓 31 個進出口全部走這一支。**

錯誤訊息刻意一律是「找不到這個行程，或你沒有存取權限」，**不區分「行程不存在」
與「你不是成員」**——區分開來等於讓任何人拿 cuid 去探測哪些行程存在。
有測試直接斷言這兩種情況的訊息字串相同。

P7.3 只先用它守住邀請這一區。發券等於授予權限，不能等 P7.4。

### 邀請券的三道閘門

券綁定特定團員，可重複使用，但有到期日（預設 7 天，且**不超過行程結束日**）、
可用次數上限（預設 5 次）、隨時可撤銷。token 只存 SHA-256，原始值只在產生
當下顯示一次，連團長自己都查不回來，只能撤銷後重發。

**併發認領**用條件更新處理：`updateMany` 只在 `usedCount < maxUses` 時加一，
兩個人同時打開同一張券時只有一個會成功。有測試用 `Promise.all` 同時認領，
斷言恰好一個成功、且 `TripMembership` 只有一筆指向該成員。

檢查順序是 revoked → expired → **member-taken → exhausted**。member-taken
放在 exhausted 前面是刻意的：`maxUses=1` 的券用過一次後兩者同時成立，但
「這個身分已經有人認領」才是真正的阻擋原因——講次數用完會讓團長以為重發一張
就好，而重發同樣會被拒絕。

### 兩種認領情境

- **未登入**：建立一個 `email` 與 `passwordHash` 都是 null 的帳號，顯示名稱
  沿用團員名字，當場簽發 session
- **已登入**：把現有帳號綁到這位團員，不另外建帳號也不發新 session
...
- `pnpm lint` / `typecheck` / `build` 全過
- `pnpm test` 267 passed（新增 19 條：邀請券 13、權限守門 6），regression 17/17 不變
- 瀏覽器實測完整流程：團長登入 → 成員頁看到邀請區 → 為「阿明」產生連結
  （到期日正確落在 7 天後）→ 登出模擬另一台裝置 → 開連結看到「你被邀請加入
  P73 UI 測試團，你的身分是阿明」→ 按下加入 → **以阿明身分登入並直接進到該
  行程、看得到自己的分攤小計**（這就是 P7.3 的完成定義）→ DB 確認該帳號無
  email、無密碼、角色 EDITOR、綁到正確的團員 → 以 EDITOR 身分回成員頁
  **看不到邀請區** → 換回團長，阿明已不在可邀請清單、券顯示 1/5 次 →
  撤銷 → 連結立刻失效並顯示「這張邀請已被撤銷」。測試資料已全部還原

### 這次踩到的坑

`"use server"` 檔案的**所有匯出都必須是 async 函式**。我把
